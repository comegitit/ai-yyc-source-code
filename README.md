...
| `tools/` | Repo scripts (sitemap and structured-data generators, brief sync) |
...
The structured-data generator rewrites the JSON-LD block in each page in
place, between HTML comment markers, and is safe to re-run. Every value it
writes is read from the page it writes into (`<h1>`, the meta description, the
canonical, the og:image), with dates and categories coming from `BLOG_POSTS`.
Edit the script rather than the JSON-LD in a page; hand edits are overwritten
on the next run.
...
## Briefs at the repo root

`Publishing-New-Content-Checklist.md` is the plain-language companion to the
"Adding a blog post" section above. It is **generated**, not hand-written:

```sh
python tools/sync_brief.py            # regenerate the .md from the .docx
...
directional on purpose: round-tripping through Word is lossy both ways, so
two-way sync would churn the markdown on every save with no honest way to
resolve a conflict.

The script also counts em-dashes, since Word's autocorrect inserts them and
the house style does not use them.
